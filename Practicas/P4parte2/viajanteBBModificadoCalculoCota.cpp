...
int calculaCota( vector <vector <int> > &distancias, vector <int> ciudades, vector <int> &minimas_distancias ){
  int suma_total = 0;
  ...
  if ( distancias.size() == ciudades.size() ){//Ya están todas las ciudades. Calculamos la distancia total del circuito
    ...
    for (int i = 0; i < distancias.size(); i++){//Bucle para las ciudades no están en el recorrido actual
      if (!estaCiudad(ciudades,i)){
        suma_total += minimas_distancias[i];
      }
    }
  }
  return suma_total;
...
void guardaDistanciasMinimas (vector <vector <int> > &distancias, vector <int> &minimas_distancias){
  int distancia_minima;
  for (int i = 0; i < distancias.size(); i++){//Para cada ciudad
    distancia_minima = 99999999;
    for (int j = 0; j < distancias.size(); j++){//Buscamos la distancia mínima y la almacenamos en el vector
      if ( distancias[i][j] < distancia_minima && i != j )//Se elige la distancia menor para esa ciudad que no sea la distancia hacia sí misma
        distancia_minima = distancias[i][j];
    }
    minimas_distancias.push_back(distancia_minima);
  }
}
...
// for que muestra las distancias en forma de matriz
  ...
  vector <int> minimas_distancias; //Vector para guardar la distancia mínima para cada ciudad, para ahorrar tiempo
  guardaDistanciasMinimas(distancias, minimas_distancias);
  ...
  Nodo e_nodo(calculaCota(distancias,ciudades, minimas_distancias),ciudades);
  pq.push(e_nodo);
      ...
      if(distancias.size() > e_nodo.ciudades().size()){//No es nodo hoja
        ...
        for (int i = 0; i < distancias.size();i++){//Expandimos e_nodo
          if (!estaCiudad(e_nodo.ciudades(),i)){
            ...
            cota = calculaCota(distancias,ciudades, minimas_distancias);
        ...
        /*else{
          ;//cout << "No es mejor recorrido que el anterior. Cota superior: "<<mejor_distancia_total<<"; cota inferior del nodo hoja: "<<e_nodo.cota()<< endl;
        }*/
