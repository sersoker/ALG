...
bool hayQuePodar(vector <int> &MV,vector <int> &S){
  int suma_total = 0;
  int suma_equipo1 = 0, suma_equipo2 = 0;
  for( int i = 0; i < MV.size(); i++ ){
    suma_total+=MV[i];
  }
  for( int i = 0; i < S.size(); i++){
    if(S[i]==1) suma_equipo1+=MV[i];
    else suma_equipo2+=MV[i];
  }
  if (suma_equipo1>suma_total/2 || suma_equipo2>suma_total/2)
    return true;
  else
    return false;
}
...
void particion(vector <int> &MV,vector <int> &S){
    ...
    if (!hayQuePodar(MV,S)){
        ...
        else{//"Segunda poda": Si era solución al sumarlo al equipo1, nunca va a poder ser solución si lo hubiéramos añadido al equipo2
          S.pop_back();
          S.push_back(2);
