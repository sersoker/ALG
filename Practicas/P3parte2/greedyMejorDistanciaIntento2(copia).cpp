...
bool esFactible ( vector <int> &veces_usada_ciudad, int ciudad1, int ciudad2 ){
  ...
  for ( int i = 0; i < veces_usada_ciudad.size(); i++ ){
    ...
    if ( i == ciudad1 || i == ciudad2 ) valor++; //Si en esa posición es una de las elegidas, le añadimos uno para predecir qué pasaría si añadiéramos esa ciudad
...
// for que muestra las distancias en forma de matriz
  ...
  for (int k = 0; k < distancias.size(); k++){
    ...
    cout << "Nueva distancia elegida: "<<menor_distancia << " entr las ciudades " << nueva_x << " y " <<nueva_y<< endl;
    ...
    cout << "Antes de sumar en 1 la ciudad "<< nueva_x << ": "<<veces_usada_ciudad[nueva_x]<< endl;
    veces_usada_ciudad[nueva_x]=veces_usada_ciudad[nueva_x]+1;
    cout << "Después de haber sumado 1 la ciudad "<< nueva_x << ": "<<veces_usada_ciudad[nueva_x]<< endl;
        cout << "Antes de sumar en 1 la ciudad "<< nueva_y << ": "<<veces_usada_ciudad[nueva_y]<< endl;
    veces_usada_ciudad[nueva_y]=veces_usada_ciudad[nueva_y]+1;
    cout << "Después de haber sumado 1 la ciudad "<< nueva_y << ": "<<veces_usada_ciudad[nueva_y]<< endl;
